...
public class Odometry
{
    ...
    public Odometry(double encoderRate, double baseline)
    {
        this.encoderRate = encoderRate;
        this.baseline = baseline;
    }

    public void propagate(OdometryPoint newOdom, OdometryPoint oldOdom,
            pose_t pose)
    {
        if (logger.isTraceEnabled())
        {
            ...
            logger.trace("delta right ticks: "
                    + (newOdom.getRight() - oldOdom.getRight()));
        }
        double dleft = (newOdom.getLeft() - oldOdom.getLeft()) * encoderRate;
        double dright = (newOdom.getRight() - oldOdom.getRight()) * encoderRate;

        // phi
        deltaxyt[2] = MathUtil.mod2pi((dright - dleft) / baseline);
        ...
        pose.pos[0] += (deltaxyt[0] * Math.cos(theta))
                - (deltaxyt[1] * Math.sin(theta));
        pose.pos[1] += (deltaxyt[0] * Math.sin(theta))
                + (deltaxyt[1] * Math.cos(theta));
        theta += deltaxyt[2];

        // convert theta to quat and store
        pose.orientation = LinAlg
                .rollPitchYawToQuat(new double[] { 0, 0, theta });
        ...
        if (logger.isTraceEnabled())
        {
            ...
            logger.trace(String.format(
                    "odom: n(%d,%d) o(%d,%d) p(%5.2f,%5.2f,%5.1f)", newOdom
                            .getLeft(), newOdom.getRight(), oldOdom.getLeft(),
                    oldOdom.getRight(), pose.pos[0], pose.pos[1], theta));
        }
    }
}
