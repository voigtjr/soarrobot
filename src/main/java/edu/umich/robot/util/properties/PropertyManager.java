...
public class PropertyManager
{
     ...
     * @param listener
     *            The listener
     * @return a handle that can be used to easily remove or re-add the listener
     *         later
     * @see PropertyKey
     * @see PropertyListener
     * @see PropertyListenerHandle
     */
    public <T> PropertyListenerHandle<T> addListener(PropertyKey<T> key,
     ...
     *            The value type of the property
     * @param key
     *            The key the listener was registered for
     * @param listener
     *            The listener
     */
    public <T> void removeListener(PropertyKey<T> key,
            PropertyListener<T> listener)
    {
    ...
    /**
     * Set the property provider for a particular property. A property change
     * event will be fired for the property.
     * 
     * <p>
     * This method may be called from any thread
     * 
    ...
    /**
     * Set the value of a property. A property changed event will be fired. A
     * default property provider will be created if one has not already been
     * set. If the property is readonly, an exception will be thrown.
     * 
     * <p>
     * This method may be called from any thread.
     * 
     ...
     *             if the value is not compatible with the property (out of
     *             range, null, etc)
    ...
    public <T, V extends T> T set(PropertyKey<T> key, V value)
    {
        if (key.isReadonly())
        {
            throw new UnsupportedOperationException("property '" + key
                    + "' is readonly");
        }
     ...
     *            The old value
     */
    public <T, V extends T> void firePropertyChanged(PropertyKey<T> key,
            V value, final T oldValue)
    {
        final PropertyChangeEvent<T> event = new PropertyChangeEvent<T>(key,
                oldValue, value);
    ...
    private <T> PropertyProvider<T> getProvider(PropertyKey<T> key)
    {
        synchronized (properties)
        {
            ...
            PropertyProvider<T> provider = (PropertyProvider<T>) properties
                    .get(key);
            if (provider == null)
            {
    ...
    private <T> List<PropertyListener<T>> getListenersForKey(PropertyKey<T> key)
    {
        ...
        {
            @SuppressWarnings("unchecked")
            List<PropertyListener<T>> list = (List<PropertyListener<T>>) listeners
                    .get(key);
            if (list == null)
            {
